...
def main():
    ...
    if not os.path.exists(data_file):
        raise FileNotFoundError(
            f"{data_file} not found. Run experiment_u1_2d_hmc_b.py first."
        )

    print("=== 2D U(1) gauge (Fourier slice): Flows on X and X/Λ ===")
    print(f"[LOAD] {data_file}")
    data = np.load(data_file)
    theta_raw = data["raw"]
    theta_trans = data["trans_fourier"]
    ...
    # Lift canonical samples back to X by random x-translation
    theta_nf_trans_list = []
    for i in range(n_eval):
        ...
        cfg_shift = np.roll(cfg, shift=-shift_x, axis=1)  # roll in x
    ...
    for k, v in obs_X_nf.items():
        print(f"    {k:16s} = {v:+.6f}")
    print("  Flow on X/Λ (Fourier slice, lifted):")
    for k, v in obs_trans_nf.items():
        print(f"    {k:16s} = {v:+.6f}")

    out_file = f"u1_2d_L{L}_beta{beta:.2f}_b_flows_results.npz"
    np.savez(
        out_file,
        nll_X=nll_X,
        nll_trans=nll_trans,
